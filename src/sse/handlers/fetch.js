...
export async function handleFetch(request) {
  ...
  if (permissions) {
    ...
    if (comboModels) {
      const check = checkPermission(permissions, "combo", providerInput);
      if (!check.allowed) {
        log.logAuthPermission({ keyId, kind: "combo", target: providerInput, reason: check.reason });
      ...
      const resolvedTarget = `${providerInput}/*`;

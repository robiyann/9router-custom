...
export async function handleChat(request, clientRawRequest = null) {
  ...
  // Enforce API key and retrieve permissions
  ...
  if (permissions) {
    ...
    if (comboModels) {
      const check = checkPermission(permissions, "combo", modelStr);
      if (!check.allowed) {
        log.logAuthPermission({ keyId, kind: "combo", target: modelStr, reason: check.reason });
      ...
      // Target uses original modelStr when prefixed (matches /v1/models listing format).
      // For bare aliases, fall back to resolved provider/model.

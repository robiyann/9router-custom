...
export default function ProvidersPage() {
  ...
  const [bulkDeleteProvider, setBulkDeleteProvider] = useState("all");
  const [bulkDeleteFilter, setBulkDeleteFilter] = useState("dead");
  ...
  const activeProviders = Array.from(new Set(connections.map(c => c.provider))).map(p => {
    const meta = OAUTH_PROVIDERS[p] || APIKEY_PROVIDERS[p] || FREE_PROVIDERS[p] || FREE_TIER_PROVIDERS[p] || WEB_COOKIE_PROVIDERS[p];
    const node = providerNodes.find(n => n.id === p);
    const name = node?.name || meta?.name || p;
    return { id: p, name };
  });

  const getAffectedConnectionsCount = () => {
    let list = connections;
    if (bulkDeleteProvider !== "all") {
      list = list.filter(c => c.provider === bulkDeleteProvider);
    }
    if (bulkDeleteFilter === "inactive") {
      list = list.filter(c => c.isActive === false);
    } else if (bulkDeleteFilter === "dead") {
      list = list.filter(c => c.testStatus === "error" || c.testStatus === "expired" || c.testStatus === "unavailable");
    }
    return list.length;
  };

  const handleBatchDelete = async () => {
    if (bulkDeleteFilter === "all" && bulkDeleteConfirmText.trim().toLowerCase() !== "delete") {
      alert("Please type 'delete' to confirm complete deletion.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        notify.addNotification({
          type: "success",
          message: `Successfully deleted ${data.count} connection(s).`
        });
        ...
        notify.addNotification({
          type: "error",
          message: data.error || "Failed to delete connections."
        });
      }
    } catch (error) {
      console.log("Error during batch delete:", error);
      notify.addNotification({
        type: "error",
  ...
  const globalBatchDeleteModal = (
    <Modal
      ...
      title="Batch Delete Connections"
    >
      <div className="flex flex-col gap-4 py-2">
        <p className="text-xs text-text-muted font-normal">
          Delete inactive or failed connection credentials in bulk to keep your routing pool clean.
        </p>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <select
            value={bulkDeleteProvider}
            onChange={(e) => setBulkDeleteProvider(e.target.value)}
            disabled={bulkDeleting}
            className="w-full rounded-lg border border-border bg-bg px-3 py-2 text-sm focus:outline-none focus:border-primary text-text-main"
          >
        ...
        {/* Filter selection */}
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-semibold text-text-muted">Target Connections</label>
          <select
            value={bulkDeleteFilter}
            onChange={(e) => {
              setBulkDeleteFilter(e.target.value);
              setBulkDeleteConfirmText("");
            }}
            disabled={bulkDeleting}
            className="w-full rounded-lg border border-border bg-bg px-3 py-2 text-sm focus:outline-none focus:border-primary text-text-main"
          >
            <option value="dead">Dead / Failed connections only</option>
            <option value="inactive">Disabled / Inactive connections only</option>
            <option value="all">All connections (CAUTION)</option>
          </select>
        </div>

        {/* Impact preview */}
        <div className={`p-3 rounded-lg border text-xs flex items-center gap-2.5 ${bulkDeleteFilter === "all" ? "bg-red-500/10 border-red-500/20 text-red-600 dark:text-red-400" : "bg-primary/5 border-primary/10 text-text-main"}`}>
          <span className="material-symbols-outlined text-[18px]">
            {bulkDeleteFilter === "all" ? "warning" : "info"}
          </span>
          <div>
            <p className="font-semibold text-xs leading-normal">
              Estimated impact: {getAffectedConnectionsCount()} connection(s) will be deleted.
            </p>
            {bulkDeleteFilter === "all" && (
              <p className="mt-0.5 opacity-90">This will permanently delete all connections matching the provider scope.</p>
            )}
          </div>
        </div>

        {/* Confirmation input for 'all' filter */}
        {bulkDeleteFilter === "all" && (
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-red-500">
              Type <code className="font-mono bg-red-500/20 px-1 py-0.5 rounded text-red-600 dark:text-red-400">delete</code> to confirm
            </label>
            <Input
              ...
              disabled={bulkDeleting}
              error={bulkDeleteConfirmText && bulkDeleteConfirmText.toLowerCase() !== "delete" ? "Input does not match 'delete'" : ""}
            />
          </div>
        )}
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <Button
            variant={bulkDeleteFilter === "all" ? "danger" : "primary"}
            fullWidth
            onClick={handleBatchDelete}
            disabled={
              bulkDeleting ||
              getAffectedConnectionsCount() === 0 ||
              (bulkDeleteFilter === "all" && bulkDeleteConfirmText.toLowerCase() !== "delete")
            }
          >
            {bulkDeleting ? "Deleting..." : "Delete Connections"}
          </Button>
        </div>
      </div>
    </Modal>
    ...
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:px-0">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-border-subtle pb-4">
        <div>
          ...
          <p className="text-xs text-text-muted mt-1">Manage API keys, OAuth logins, and customize routing prefixes.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link href="/dashboard/providers/prefixes">
            <Button size="sm" variant="secondary" icon="settings_ethernet">
              Prefix Manager
            </Button>
          </Link>
        </div>

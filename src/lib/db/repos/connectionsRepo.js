...
export async function batchDeleteProviderConnections({ providerId, filter }) {
  const db = await getAdapter();
  let count = 0;
  db.transaction(() => {
    ...
    let rows = db.all(sql, params).map(rowToConn);

    if (filter === "dead") {
      rows = rows.filter(c => c.testStatus === "error" || c.testStatus === "expired" || c.testStatus === "unavailable");
    }
    ...
    for (const conn of rows) {
      db.run(`DELETE FROM providerConnections WHERE id = ?`, [conn.id]);
      affectedProviders.add(conn.provider);
      count++;
    }

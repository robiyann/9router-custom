...
export default function EditConnectionModal({ isOpen, connection, proxyPools, onSave, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isAzure) {
        providerSpecific.azureEndpoint = azureData.azureEndpoint;
        providerSpecific.apiVersion = azureData.apiVersion;
        providerSpecific.deployment = azureData.deployment;
        providerSpecific.organization = azureData.organization;
      }
      if (isCloudflareAi) {
        providerSpecific.accountId = cloudflareData.accountId;
      }

      if (formData.prefix !== undefined) {
        const cleanPrefix = formData.prefix.trim();
        if (cleanPrefix) {
      ...
      updates.providerSpecificData = providerSpecific;
  ...
  return (
    <Modal isOpen={isOpen} title="Edit Connection" onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        />
        <Input
          label="Custom Routing Prefix"
          value={formData.prefix}
          onChange={(e) => setFormData({ ...formData, prefix: e.target.value })}
          placeholder={connection ? getProviderAlias(connection.provider) : ""}
          hint="Customize the routing prefix for models of this connection. Leave blank for default."
        />
